...
def client_program():
    ...
    while True:
        message = input("[CLIENT]> Enter plain text : ")
        encrypted_msg = enc(key, bytes(message, 'utf-8'))
        if message.lower().strip() == 'stop':
            break
        print("[CLIENT]> Sent encrypted text to server:", encrypted_msg)
        ...
        print("[SERVER]> Received iteration over text from Server:", data)
